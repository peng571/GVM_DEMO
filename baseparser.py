...
class Builder:
    ...
    def _from_kvlist(self, maker, root, key, values, parent_kwargs):
        ...
        context = {'creator_type':self.creator_type}
        def do_beh(beh):
            if not parts:
                raise NodeError('need more part', _parts[-1])
            return beh.do(maker, root, self, parts.pop(0), init_kwargs, context)
        ...
        obj = self.common_build(maker, root, context['creator_type'], key, values, init_kwargs)
        parent_kwargs.setdefault(self.k_belong, []).append(obj)
        return True
        
    def from_kvlist(self, maker, root, key, values, parent_kwargs):
        try:
            return self._from_kvlist(maker, root, key, values, parent_kwargs)
        except Exception as e:
            exc_class, exc, tb = sys.exc_info()
            newexc = NodeError(str(e), key)
            raise newexc.with_traceback(tb) from None
            
    def common_build(self, maker, root, creator_type, name, kwlist, init_kwargs=None):
        #print(kwlist)
        assert isinstance(kwlist, ListNode)
        args = []
        build_kwargs = DictNode()
        build_kwargs.set_span(name.start_mark, name.end_mark)
        build_kwargs.update(copy.deepcopy(creator_type.init_kwargs))
        build_kwargs.update(init_kwargs or {})
        kwlist_kwargs = DictNode()
        
        def from_kvlist(k, v):
            if self.get_locale_builder():
                return self.get_locale_builder().locale_builder.from_kvlist(maker, root, k, v, build_kwargs)
            return False
            ...
            if attr not in extra:
                if not(extra and creator_type.NO_CHECK in extra):
                    if attr in kwlist_kwargs:
                        ...
                        print(creator_type.typename, creator_type.fields, build_kwargs)
        ...
        pack = creator_type.pack(maker, root, args, build_kwargs)
        return pack
...
class MultiBuilder(Builder):
    ...
    def from_kvlist(self, maker, root, key, values, parent_kwargs):
        for builder in self.builders:
            obj = builder.from_kvlist(maker, root, key, values, parent_kwargs)
            if obj:
                return True
        return False
...
class AsNameBehavior(Behavior):
    def do(self, maker, root, builder, text, init_kwargs, context):
        init_kwargs['name'] = text
        return True
...
class KSubBehavior(Behavior):
    ...
    def do(self, maker, root, builder, text, init_kwargs, context):
        creator_type, kwlist = self.__class__.text_match(text, self.subclasses)
        if creator_type:
            sub = builder.common_build(maker, root, creator_type, text, ListNode(), kwlist)
...
class BuildBehavior(Behavior):
    ...
    def do(self, maker, root, builder, text, init_kwargs, context):
        if self.builder.from_kvlist(maker, root, text, ListNode(), init_kwargs):
            return True
        elif self.strict:
            raise NodeError('找不到可符合的類別', text)
        return False
        
class PropertyBahavior(Behavior):
    def __init__(self, subclasses, **kwargs):
        super().__init__(**kwargs)
        self.subclasses = subclasses
...
CreatorPack = namedtuple('CreatorPack', 'creator_type maker root args kwargs')
...
class CreatorType:
    ...
    def resolve_children(self, target, parent=None):
        if isinstance(target, list):
            node = ListNode(self.resolve_children(v, parent) for v in target)
            if hasattr(target, 'start_mark'):
                node.set_span(target.start_mark, target.end_mark)
            return node
        if isinstance(target, dict):
            node = DictNode((k, self.resolve_children(v, parent)) for k, v in target.items())
            if hasattr(target, 'start_mark'):
                node.set_span(target.start_mark, target.end_mark)
            return node
    ...
    def resolve(self, pack, parent=None):
        obj = DictNode(pack.kwargs)
        obj.set_span(pack.kwargs.start_mark, pack.kwargs.end_mark)
...
class Maker:
    ...
    def from_file(self, filename):
        ...
        pack = self.builder.common_build(self, root, self.Root, StrNode(filename), data)
        return pack
        
    def from_main_file(self, filename):
        import traceback
        try:
